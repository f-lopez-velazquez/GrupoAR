...
<head>
    ...
    <style>
      ...
      .logoTitle { font-size:1.85em; font-weight:700; color:var(--azul);}
  ...
  .form-row { flex-direction:column; gap:3px; margin-bottom:2px; }
  .form-col { min-width:90px; margin-bottom:2px; }
  .form-container, .preview-container { padding:7px 2px 8px 2px;}
  table, th, td { font-size:.97em;}
  #previewResponsive { font-size:1em;}
  header img { height:38px;}
  .logoTitle { font-size:1em;}
  label { font-size: .99em; margin-bottom: 0.5px;}
  input, select, textarea { padding: 7px 7px 7px 33px; font-size:.98em; }
  .ai-tools-bar, .historico-bar { margin-bottom: 4px;}
}
...
<div class="container">
  ...
  // ==== IA: mejora de redacción (simulada) ====
...
async function generarPDFpro() {
  ...
  doc.addImage(logo, 'JPEG', (w-90)/2, (h-120)/2, 90, 120);
  doc.restoreGraphicsState();

  // --- Encabezado ---
  // Logo grande a la izquierda
  let logoH = 38, logoW = 30;
  doc.addImage(logo, 'JPEG', margin, margin, logoW, logoH);

  // "GRUPO AR" pequeño y centrado vertical a logo
  doc.setFont('helvetica','bold');
  doc.setFontSize(13.2);
  doc.setTextColor(10, 110, 200);
  doc.text("GRUPO", margin + logoW + 8, margin + logoH/2 - 2.3);
  doc.setTextColor(220, 30, 40);
  doc.text("AR", margin + logoW + 30, margin + logoH/2 - 2.3);

  // Datos cliente/ubicación/fecha alineados a la derecha, simétrico
  ...
  doc.setFont('helvetica', 'bold');
  doc.text("Cliente:", w - margin - 45, encY + encLine, { align: 'right' });
  doc.text("Ubicación:", w - margin - 45, encY + 2*encLine, { align: 'right' });
  doc.setFont('helvetica', 'normal');
  doc.text(document.getElementById('client').value || '--', w - margin, encY + encLine, { align: 'right' });
  doc.text(document.getElementById('location').value || '--', w - margin, encY + 2*encLine, { align: 'right' });
  doc.text("Fecha: " + (new Date().toLocaleDateString()), w - margin, encY + 3*encLine, { align: 'right' });

  // Línea divisoria SUTIL debajo de encabezado
  let y = Math.max(margin + logoH, encY + 3*encLine + 5) + 7;
  ...
  // Tabla encabezado
  let th = 9; // altura de fila encabezado
  let td = 7.3; // altura de fila de datos
  if (tipo === 'express') {
    doc.rect(margin, y, w-margin*2, th, 'F');
    doc.text("Descripción", margin+3, y+6);
    doc.text("Precio", w-margin-50, y+6, {align:'right'});
    doc.text("Total", w-margin-10, y+6, {align:'right'});
    y += th+1.5;
    doc.setFont('helvetica','normal');
    doc.setFontSize(10);
    doc.setTextColor(34,34,34);

    // Filas de items (mejor espacio)
    items.forEach(it=>{
      doc.text(it.desc, margin+3, y+5);
      doc.text(`$${(it.pr||0).toFixed(2)}`, w-margin-50, y+5, {align:'right'});
      doc.text(`$${(it.pr||0).toFixed(2)}`, w-margin-10, y+5, {align:'right'});
      y+=td;
    });
  } else {
    doc.rect(margin, y, w-margin*2, th, 'F');
    doc.text("Descripción", margin+3, y+6);
    doc.text("Cant.", w-margin-74, y+6, {align:'right'});
    doc.text("Material", w-margin-55, y+6, {align:'right'});
    doc.text("Mano Obra", w-margin-28, y+6, {align:'right'});
    doc.text("Total", w-margin-10, y+6, {align:'right'});
    y += th+1.5;
    doc.setFont('helvetica','normal');
    doc.setFontSize(10);
    ...
    items.forEach(it=>{
      ...
      doc.text(it.desc, margin+3, y+5);
      doc.text(`${it.qt||1}`, w-margin-74, y+5, {align:'right'});
      doc.text(`$${(it.ma||0).toFixed(2)}`, w-margin-55, y+5, {align:'right'});
      doc.text(`$${(it.lb||0).toFixed(2)}`, w-margin-28, y+5, {align:'right'});
      doc.text(`$${tot.toFixed(2)}`, w-margin-10, y+5, {align:'right'});
      y+=td;
    });
  }
  ...
  let totalBlockY = y;
  doc.setFont('helvetica','normal'); doc.setFontSize(10.6);
  doc.setTextColor(30,30,30);
  doc.text(`Subtotal: $${subtotal.toFixed(2)}`, w-margin-1, totalBlockY, {align:'right'});
  doc.text(`IVA: $${iva.toFixed(2)}`, w-margin-1, totalBlockY+6, {align:'right'});
  doc.setFont('helvetica','bold');
  doc.setFontSize(11.5);
  doc.text(`Total: $${total.toFixed(2)}`, w-margin-1, totalBlockY+12, {align:'right'});
  if (anticipo > 0) {
    doc.setFont('helvetica','normal');
    doc.setFontSize(10.2);
    doc.text(`Anticipo: $${anticipo.toFixed(2)}`, w-margin-1, totalBlockY+18, {align:'right'});
    doc.setFont('helvetica','bold');
    doc.setFontSize(11.5);
    doc.text(`Saldo: $${saldo}`, w-margin-1, totalBlockY+24, {align:'right'});
    y+=12;
  }
  ...
  if (detalles.length) {
    y = Math.max(y, totalBlockY+26) + 13;
    doc.setFont('helvetica','bold');
    doc.setFontSize(11.5);
    doc.setTextColor(10,110,200);
    doc.text("Detalles y Especificaciones:", margin+2, y);
    y += 7;
    doc.setFont('helvetica','normal');
    doc.setFontSize(10);
  ...
  doc.setTextColor(10, 110, 200);
  doc.text('Grupo AR', w/2, y+7.7, {align:'center'});
  doc.setFont('helvetica','normal'); doc.setFontSize(8.2); doc.setTextColor(150, 150, 150);
  doc.text('Firma simbólica digital — No requiere firma autógrafa.', w/2, y+13, {align:'center'});
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function() {
    navigator.serviceWorker.register('sw.js').then(function(reg) {
      ...
      // Fallo al registrar service worker
    });
  });
}
</script>
